...
nk_sdl_render(enum nk_anti_aliasing AA)
{
...

    {
        ...
        nk_draw_foreach(cmd, &sdl.ctx, &dev->cmds)
        {
            ...
            {
                SDL_Rect r;
                r.x = static_cast<int>(cmd->clip_rect.x);
                r.y = static_cast<int>(cmd->clip_rect.y);
                r.w = static_cast<int>(cmd->clip_rect.w);
                r.h = static_cast<int>(cmd->clip_rect.h);
#ifdef NK_SDL_CLAMP_CLIP_RECT
                if (r.x < 0) {
                    r.w += r.x;
                    r.x = 0;
...

            {
                ...
                SDL_RenderGeometryRaw(sdl.renderer,
                        ...
                        static_cast<int>((vbuf.needed / vs)),
